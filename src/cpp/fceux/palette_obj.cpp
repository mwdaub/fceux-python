...
void Palette::ApplyDeemphasisNTSC(int entry, u8& r, u8& g, u8& b)
{
...
void Palette::SetUserPalette(uint8 *pal, int nEntries)
{
	if(!pal)
	{
...
void Palette::LoadGamePalette(void)
{
	palette_game_available = false;
	std::string path = fceu->file.MakeFName(FCEUMKF_PALETTE,0,0);
	FILE* fp = fceu::UTF8fopen(path,"rb");
	if(fp)
	{
...
void Palette::SetNTSCTH(bool en, int tint, int hue)
{
	ntsctint=tint;
	ntschue=hue;
	ntsccol_enable = en;
...
void Palette::SetNESDeemph_OldHacky(uint8 d, int force)
{
	static uint16 rtmul[]={
...
void Palette::CalculatePalette(void)
{
...
void Palette::ChoosePalette(void)
{
	...
	else if(ntsccol_enable && !fceu->PAL && fceu->GameInfo->type!=GIT_VSUNI)
	{
...
void Palette::WritePalette(void)
{
	int x;
...
void Palette::NTSCDEC(void)
{
	if(ntsccol_enable && fceu->GameInfo->type!=GIT_VSUNI &&!fceu->PAL && fceu->GameInfo->type!=GIT_NSF)
	{
		int which;
		if(controlselect)
		{
...
void Palette::NTSCSELHUE(void)
{
	if(ntsccol_enable && fceu->GameInfo->type!=GIT_VSUNI && !fceu->PAL && fceu->GameInfo->type!=GIT_NSF){controlselect=1;controllength=360;}
}

void Palette::NTSCSELTINT(void)
{
	if(ntsccol_enable && fceu->GameInfo->type!=GIT_VSUNI && !fceu->PAL && fceu->GameInfo->type!=GIT_NSF){controlselect=2;controllength=360;}
}

void Palette::DrawNTSCControlBars(uint8 *XBuf)
{
	uint8 *XBaf;
	int which=0;
	int x,x2;
	...
	if(controlselect==1)
	{
		fceu->drawing.DrawTextTrans(XBuf+128-12+180*256, 256, (uint8 *)"Hue", 0x85);
		which=ntschue<<1;
	}
	else if(controlselect==2)
	{
		fceu->drawing.DrawTextTrans(XBuf+128-16+180*256, 256, (uint8 *)"Tint", 0x85);
		which=ntsctint<<1;
	}
