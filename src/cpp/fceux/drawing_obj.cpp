...
void Drawing::DrawTextLineBG(uint8 *dest)
{
	int x,y;
	//100,40,15,10,7,5,2};
	for(y=0;y<14;y++)
...
void Drawing::DrawMessage(bool beforeMovie)
{
	if(guiMessage.howlong)
	{
		...
		if (guiMessage.linesFromBottom > 0)
			t=XBuf+fceu->TextScanlineOffsetFromBottom(guiMessage.linesFromBottom)+1;
		else
			t=XBuf+fceu->TextScanlineOffsetFromBottom(20)+1;
	...
	if(subtitleMessage.howlong)
	{
		...
		tt=XBuf+fceu->TextScanlineOffsetFromBottom(216);
...
void Drawing::drawstatus(uint8* XBuf, int n, int y, int xofs)
{
	uint8* slines=sline_icons[n];
	int i;


	XBuf += fceu->TextScanlineOffsetFromBottom(y) + 240 + 255 + xofs;
...
void Drawing::DrawRecordingStatus(uint8* XBuf)
{
	if(false)
	{
		...
		else if(fceu->movie.Mode(MOVIEMODE_PLAY|MOVIEMODE_FINISHED))
		{
...
void Drawing::DrawNumberRow(uint8 *XBuf, int *nstatus, int cur)
{
	uint8 *XBaf;
	int z,x,y;

	XBaf=XBuf - 4 + (fceu->FSettings.LastSLine-34)*256;
	if(XBaf>=XBuf)
		for(z=1;z<11;z++)
		{
			if(nstatus[z%10])
...
void Drawing::DrawTextTransWH(uint8 *dest, int width, uint8 *textmsg, uint8 fgcolor, int max_w, int max_h, int border)
{
...
void Drawing::DrawTextTrans(uint8 *dest, uint32 width, uint8 *textmsg, uint8 fgcolor)
{
