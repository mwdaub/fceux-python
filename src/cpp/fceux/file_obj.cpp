...
void ApplyIPS(FILE *ips, FCEUFILE* fp)
{
	...
	char* buf = (char*)fceu::dmalloc(fp->size);
	memcpy(buf,fp->EnsureMemorystream()->buf(),fp->size);


	fceu::printf(" Applying IPS...\n");
	...
	while(fread(header,1,3,ips)==3)
	{
		...
		{
			fceu::printf(" IPS EOF:  Did %d patches\n\n",count);
			goto end;
		}
		...
		if(!size)	/* RLE */
		{
			...
			//fceu::printf("  Offset: %8d  Size: %5d RLE\n",offset,size);
			...
			if((offset+size)>(uint32)fp->size)
			{
				...
				if(!buf)
				{
					fceu::printf("  Oops.  IPS patch %d(type RLE) goes beyond end of file.  Could not allocate memory.\n",count);
					goto end;
				}
		...
		else		/* Normal patch */
		{
			...
			if((offset+size)>(uint32)fp->size)
			{
				...
				if(!buf)
				{
					fceu::printf("  Oops.  IPS patch %d(type normal) goes beyond end of file.  Could not allocate memory.\n",count);
					goto end;
				}
				memset(buf+fp->size,0,offset+size-fp->size);
			}
...
std::string MakeIpsFilename(FileBaseInfo fbi) {
...
FileBaseInfo File::CurrentFileBase() {
...
FCEUFILE * fopen(const char *path, const char *ipsfn, char *mode, char *ext, int index, const char** extensions)
{
	FILE *ipsfile=0;
	FCEUFILE *fceufp=0;
	...
	{
		fceu::PrintError("invalid file open mode specified (only wb and rb are supported)");
		return 0;
	}

	std::string archive,fname,fileToOpen;
	fceu::SplitArchiveFilename(path,archive,fname,fileToOpen);


	//try to setup the ips file
	if(ipsfn && read)
		ipsfile=fceu::UTF8fopen(ipsfn,"rb");
	if(read)
	{
		ArchiveScanRecord asr = ScanArchive(fileToOpen);
		asr.files.FilterByExtension(extensions);
		if(!asr.isArchive())
		{
			//if the archive contained no files, try to open it the old fashioned way
			EMUFILE_FILE* fp = EMUFILE_FILE::UTF8_fstream(fileToOpen,mode);
			...
			fceufp->archiveIndex = -1;
			fceufp->stream = fp;
			fceu::fseek(fceufp,0,SEEK_END);
			fceufp->size = fceu::ftell(fceufp);
			fceu::fseek(fceufp,0,SEEK_SET);
			goto applyips;
		}
		else
		{
			...
			if(archive == "")
				if(index != -1)
					fceufp = OpenArchiveIndex(asr, fileToOpen, index);
				else
					fceufp = OpenArchive(asr, fileToOpen, 0);
			else
				fceufp = OpenArchive(asr, archive, &fname);
	...
	applyips:
		...
		if(!ipsfile && !ipsfn)
			ipsfile=fceu::UTF8fopen(fceu::MakeIpsFilename(DetermineFileBase(fceufp->logicalPath.c_str())),"rb");
...
int fclose(FCEUFILE *fp)
{
	delete fp;
	return 1;
}

uint64 fread(void *ptr, size_t size, size_t nmemb, FCEUFILE *fp)
{
	return fp->stream->fread((char*)ptr,size*nmemb);
}

uint64 fwrite(void *ptr, size_t size, size_t nmemb, FCEUFILE *fp)
{
...
int fseek(FCEUFILE *fp, long offset, int whence)
{
	fp->stream->fseek(offset,whence);

	return fceu::ftell(fp);
}

uint64 ftell(FCEUFILE *fp)
{
	return fp->stream->ftell();
}

int read16le(uint16 *val, FCEUFILE *fp)
{
	return read16le(val,fp->stream);
}

int read32le(uint32 *Bufo, FCEUFILE *fp)
{
	return read32le(Bufo, fp->stream);
}

int fgetc(FCEUFILE *fp)
{
	return fp->stream->fgetc();
}

uint64 fgetsize(FCEUFILE *fp)
{
	return fp->size;
}

int fisarchive(FCEUFILE *fp)
{
...
void File::SetBaseDirectory(std::string const & dir)
{
	BaseDirectory = dir;
}
...
{
	//	fceu::PrintError("odirs[%d]=%s->%s", which, odirs[which], n);
	...
	static int asprintf(char **strp, const char *fmt, ...)
	{
		...
		va_start(ap,fmt);
		if(!(*strp=(char*)fceu::dmalloc(2048))) //mbg merge 7/17/06 cast to char*
			return(0);
...
std::string File::GetPath(int type)
{
	switch (type)
	{
		case FCEUMKF_STATE:
...
std::string File::MakePath(int type, const char* filebase)
{
	char ret[FILENAME_MAX];
...
std::string File::MakeFName(int type, int id1, const char *cd1)
{
	char ret[FILENAME_MAX] = "";
	struct stat tmpstat;
	...
	switch(type)
	{
		...
		case FCEUMKF_CHEAT:
			...
			break;
		case FCEUMKF_IPS:
			strcpy(ret,fceu::MakeIpsFilename(CurrentFileBase()).c_str());
			break;
...
void File::GetFileBase(const char *f)
{
...
bool isFileInArchive(const char *path)
{
	...
	FCEUFILE* fp = fceu::fopen(path,0,"rb",0,0);
	if(fp) {
