...
void Cheat::CheatAddRAM(int s, uint32 A, uint8 *p)
{
	uint32 AB=A>>10;
	int x;
...
uint8 Cheat::SubCheatsRead(uint32 A)
{
...
void Cheat::RebuildSubCheats(void)
{
	int x;
	struct CHEATF *c=cheats;
	for(x=0;x<numsubcheats;x++)
		fceu->handler.SetReadHandler(SubCheats[x].addr,SubCheats[x].addr,SubCheats[x].PrevRead);
	...
	while(c)
	{
		...
		{
			if(fceu->handler.GetReadHandler(c->addr)==&SubCheatsRead_)
			{
			...
			{
				SubCheats[numsubcheats].PrevRead=fceu->handler.GetReadHandler(c->addr);
				SubCheats[numsubcheats].addr=c->addr;
				SubCheats[numsubcheats].val=c->val;
				SubCheats[numsubcheats].compare=c->compare;
				fceu->handler.SetReadHandler(c->addr,c->addr,&SubCheatsRead_);
				numsubcheats++;
			}
		}
		c=c->next;
...
void Cheat::PowerCheats()
{
	numsubcheats=0;	/* Quick hack to prevent setting of ancient read addresses. */
	RebuildSubCheats();
}

void Cheat::CheatMemErr(void)
{
    fceu::PrintError("Error allocating memory for cheat data.");
}

/* This function doesn't allocate any memory for "name" */
int Cheat::AddCheatEntry(char *name, uint32 addr, uint8 val, int compare, int status, int type)
{
	struct CHEATF *temp;
	if(!(temp=(struct CHEATF *)fceu::dmalloc(sizeof(struct CHEATF))))
	{
		CheatMemErr();
		return(0);
	}
...
void Cheat::LoadGameCheats(FILE *override)
{
	FILE *fp;
	unsigned int addr;
	unsigned int val;
    ...
    fceu::DispMessage("Cheats file loaded.",0); //Tells user a cheats file was loaded.
	while(fgets(linebuf,2048,fp)>0)
	{
		...
		if(doc)
		{
			...
			if(sscanf(tbuf,"%04x%*[:]%02x%*[:]%02x",&addr,&val,&compare)!=3)
				continue;
			if (!(namebuf=(char *)fceu::dmalloc(strlen(neo)+1)))
                return;
			strcpy(namebuf,neo);
		}
		else
...
void Cheat::FlushGameCheats(FILE *override, int nosave)
{
	if(CheatComp)
	{
		free(CheatComp);
	...
	else
	{
		...
		if(cheats)
		{
			...
			if(fp)
			{
				...
				fceu::PrintError("Error saving cheats.");
			cheats=cheatsl=0;
		}
		else if(!override)
			remove(fn);
...
int Cheat::AddCheat(const char *name, uint32 addr, uint8 val, int compare, int type)
{
	char *t;

	if(!(t=(char *)fceu::dmalloc(strlen(name)+1)))
	{
		CheatMemErr();
		return(0);
	}
...
int Cheat::DelCheat(uint32 which)
{
	struct CHEATF *prev;
	struct CHEATF *cur;
	uint32 x=0;
...
void Cheat::ApplyPeriodicCheats(void)
{
...
void Cheat::ListCheats(int (*callb)(char *name, uint32 a, uint8 v, int compare, int s, int type, void *data), void *data)
{
	struct CHEATF *next=cheats;
...
int Cheat::GetCheat(uint32 which, char **name, uint32 *a, uint8 *v, int *compare, int *s, int *type)
{
	struct CHEATF *next=cheats;
	uint32 x=0;
...
int Cheat::DecodeGG(const char *str, int *a, int *v, int *c)
{
	uint16 A;
	uint8 V,C;
	uint8 t;
...
int Cheat::DecodePAR(const char *str, int *a, int *v, int *c, int *type)
{
	int boo[4];
...
int Cheat::SetCheat(uint32 which, const char *name, int32 a, int32 v, int c, int s, int type)
{
	struct CHEATF *next=cheats;
	uint32 x=0;
...
int Cheat::ToggleCheat(uint32 which)
{
	struct CHEATF *next=cheats;
	uint32 x=0;
...
int Cheat::InitCheatComp(void)
{
	uint32 x;

	CheatComp=(uint16*)fceu::dmalloc(65536*sizeof(uint16));
	if(!CheatComp)
	{
		CheatMemErr();
		return(0);
...
void Cheat::CheatSearchSetCurrentAsOriginal(void)
{
	uint32 x;
...
void Cheat::CheatSearchShowExcluded(void)
{
	uint32 x;
...
int32 Cheat::CheatSearchGetCount(void)
{
	uint32 x,c=0;
...
void Cheat::CheatSearchGet(int (*callb)(uint32 a, uint8 last, uint8 current, void *data),void *data)
{
	uint32 x;
...
void Cheat::CheatSearchGetRange(uint32 first, uint32 last, int (*callb)(uint32 a, uint8 last, uint8 current))
{
	uint32 x;
	uint32 in=0;
...
void Cheat::CheatSearchBegin(void)
{
	uint32 x;
...
void Cheat::CheatSearchEnd(int type, uint8 v1, uint8 v2)
{
	uint32 x;
...
int Cheat::CheatGetByte(uint32 A)
{
	if(A < 0x10000) {
		uint32 ret;
		fceu->ppu.set_fceuindbg(1);
		ret = (*(fceu->handler.ARead[A]))(A);
		fceu->ppu.set_fceuindbg(0);
		return ret;
	} else
		return 0;
}

void Cheat::CheatSetByte(uint32 A, uint8 V)
{
   if(CheatRPtrs[A>>10])
    CheatRPtrs[A>>10][A]=V;
   else if(A < 0x10000)
    (*(fceu->handler.BWrite[A]))(A, V);
}

void Cheat::UpdateFrozenList(void)
{
