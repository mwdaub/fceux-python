...
uint8 iNES::TrainerRead(uint32 A) {
	return(trainerpoo[A & 0x1FF]);
}

void iNES::iNES_ExecPower() {
	if (CHRRAMSize != -1)
		fceu->MemoryRand(VROM, CHRRAMSize);
	...
	if (trainerpoo) {
		int x;
		for (x = 0; x < 512; x++) {
			fceu->x6502.DMW(0x7000 + x, trainerpoo[x]);
			if (fceu->x6502.DMR(0x7000 + x) != trainerpoo[x]) {
				fceu->handler.SetReadHandler(0x7000, 0x71FF, &TrainerRead_);
				break;
			}
		}
	}
...
void iNES::iNESGI(GI h) { //bbit edited: removed static keyword
	switch (h) {
	case GI_RESETSAVE:
		fceu->cart.ClearGameSave(&iNESCart);
		break;
	...
	case GI_CLOSE:
	{
		fceu->cart.SaveGameSave(&iNESCart);
		if (iNESCart.Close)
			iNESCart.Close();
		if (ROM) {
			free(ROM);
...
void iNES::SetInput(void) {
	...
	while (moo[x].input1 >= 0 || moo[x].input2 >= 0 || moo[x].inputfc >= 0) {
		if (moo[x].crc32 == iNESGameCRC32) {
			fceu->GameInfo->input[0] = moo[x].input1;
			fceu->GameInfo->input[1] = moo[x].input2;
			fceu->GameInfo->inputfc = moo[x].inputfc;
			break;
		}
		x++;
	}
...
{
    #include "ines-bad_obj.h"
};

void iNES::CheckBad(uint64 md5partial) {
	int32 x = 0;
	while (BadROMImages[x].name) {
		if (BadROMImages[x].md5partial == md5partial) {
            fceu::PrintError("The copy game you have loaded, \"%s\", is bad, and will not work properly in FCEUX.", BadROMImages[x].name);
			return;
		}
		x++;
	}
...
int iNES::iNESLoad(const char *name, FCEUFILE *fp, int OverwriteVidMode) {
	...
	if ((ROM = (uint8*)fceu::malloc(ROM_size << 14)) == NULL)
		return 0;
	memset(ROM, 0xFF, ROM_size << 14);

	if (VROM_size) {
		if ((VROM = (uint8*)fceu::malloc(VROM_size << 13)) == NULL) {
			free(ROM);
			ROM = NULL;
			return 0;
		}
	...
	if (head.ROM_type & 4) {	/* Trainer */
		trainerpoo = (uint8*)fceu::gmalloc(512);
        fceu::fread(trainerpoo, 512, 1, fp);
	}

	fceu->cart.ResetCartMapping();
	ResetExState(0, 0);

	fceu->cart.SetupCartPRGMapping(0, ROM, ROM_size << 14, 0);

    fceu::fread(ROM, 0x4000, (round) ? ROM_size : not_round_size, fp);

	if (VROM_size)
		fceu::fread(VROM, 0x2000, VROM_size, fp);
	...
	memcpy(&fceu->GameInfo->MD5, &iNESCart.MD5, sizeof(iNESCart.MD5));

	iNESCart.CRC32 = iNESGameCRC32;

    fceu::printf(" PRG ROM:  %3d x 16KiB\n", (round) ? ROM_size: not_round_size);
    fceu::printf(" CHR ROM:  %3d x  8KiB\n", head.VROM_size);
    fceu::printf(" ROM CRC32:  0x%08lx\n", iNESGameCRC32);
	{
		int x;
        fceu::printf(" ROM MD5:  0x");
		for(x=0;x<16;x++)
			fceu::printf("%02x",iNESCart.MD5[x]);
        fceu::printf("\n");
	}
    ...
    fceu::printf(" Mapper #:  %d\n", MapperNo);
    fceu::printf(" Mapper name: %s\n", mappername);
    fceu::printf(" Mirroring: %s\n", Mirroring == 2 ? "None (Four-screen)" : Mirroring ? "Vertical" : "Horizontal");
    fceu::printf(" Battery-backed: %s\n", (head.ROM_type & 2) ? "Yes" : "No");
    fceu::printf(" Trained: %s\n", (head.ROM_type & 4) ? "Yes" : "No");
	if(iNES2) 
	{
        fceu::printf(" NES2.0 Extensions\n");
        fceu::printf(" Sub Mapper #: %d\n", iNESCart.submapper);
        fceu::printf(" Total WRAM size: %d\n", iNESCart.wram_size + iNESCart.battery_wram_size);
        fceu::printf(" Total VRAM size: %d\n", iNESCart.vram_size + iNESCart.battery_vram_size);
		if(head.ROM_type & 2)
		{
            fceu::printf(" WRAM backed by battery: %d\n", iNESCart.battery_wram_size);
            fceu::printf(" VRAM backed by battery: %d\n", iNESCart.battery_vram_size);
		}
	}
	...
	*/
	if (VROM_size)
		fceu->cart.SetupCartCHRMapping(0, VROM, VROM_size * 0x2000, 0);

	if (Mirroring == 2) {
		ExtraNTARAM = (uint8*)fceu::gmalloc(2048);
		fceu->cart.SetupCartMirroring(4, 1, ExtraNTARAM);
	} else if (Mirroring >= 0x10)
		fceu->cart.SetupCartMirroring(2 + (Mirroring & 1), 1, 0);
	else
		fceu->cart.SetupCartMirroring(Mirroring & 1, (Mirroring & 4) >> 2, 0);

	iNESCart.battery = (head.ROM_type & 2) ? 1 : 0;
	iNESCart.mirror = Mirroring;

	if (!iNES_Init(MapperNo))
		fceu::PrintError("iNES mapper #%d is not supported at all.", MapperNo);
	...
	if (iNES2) {
		fceu->SetVidSystem(((head.TV_system & 3) == 1) ? 1 : 0);
...
int iNES::iNesSaveAs(char* name)
{
	...
	if (fceu->GameInfo->type != GIT_CART) return 0;
	if (fceu->GameInterface != &iNESGI_) return 0;
	...
	while (tmp->init) {
		if (num == tmp->number) {
			...
			if (!VROM_size) {
				...
				if ((VROM = (uint8*)fceu::dmalloc(CHRRAMSize)) == NULL) return 0;
                fceu->MemoryRand(VROM, CHRRAMSize);
				...
				{
					fceu->cart.SetupCartCHRMapping(0, VROM, CHRRAMSize, 1);
